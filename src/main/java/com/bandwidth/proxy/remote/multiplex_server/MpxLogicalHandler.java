...
class MpxLogicalHandler extends ChannelDuplexHandler {
        ...
        Runnable task = new Runnable() {
            ...
            public void run() {
                if (channel.isWritable()){
                    for (Map.Entry<Number, SocketChannel> entry : map.entrySet()) {
                        entry.getValue().read();
                    }
                }
                if (channel.isActive()){
                    ctx.executor().schedule(this, 1, TimeUnit.MILLISECONDS);
                }
            }
        };
        ctx.executor().schedule(task, 1, TimeUnit.MILLISECONDS);
    }
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) {
        ...
        if (pm.id instanceof Long){
            Integer[] ids = MsgUtils.proxyChannelId((Long) (pm.id));
            pm = new ProxyMsg(pm.ope, ids[1], pm.msg);
        }
        try {
